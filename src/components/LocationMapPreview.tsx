...
const LocationMapPreview = ({
  ...
  useEffect(() => {
    ...
    // 👇 Estimate typical house footprint (~50px–120px wide at zoom 19)
    const housePixelWidth = 80; // avg house width in pixels
    const houseWidthMeters = housePixelWidth * metersPerPixel;

    // Assume square-ish plot
    const plotAreaSqMeters = houseWidthMeters * houseWidthMeters * 2;
    // multiplier 2 = includes front/back yard

    let lawnSqMeters = plotAreaSqMeters * 0.6;
    // assume 60% of plot is lawn

    let lawnSqFt = lawnSqMeters * 10.7639;

    // 🧠 Adjust based on location type (simple heuristic)
  ...
  }, [finalArea, onAreaChange]);

  // 🔢 Calculator
  const calculateArea = () => {
    if (length && width) {
      const area = parseInt(length) * parseInt(width);
      setManualSize(area.toString());
      onAreaChange?.(area);
    }
  };
        ...
        {finalArea &&  (
          <div className="px-4 py-4 border-t space-y-3">
            <p className="text-center text-sm text-gray-700">
              <span className="font-semibold">Estimated lawn size:</span>{" "}
